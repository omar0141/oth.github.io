...
import 'package:shakosh/Data/Remote/OrdersRemote.dart';
...
class OrdersBloc extends Bloc<OrdersEvent, OrdersState> {
  ...
  List<String> statusTypes = ["processing", "shipping", "arrived", "canceled"];
  ...
  Future<void> deleteOrder(
      Emitter<OrdersState> emit, DeleteOrderEvent event) async {
    emit(OrdersLoading(
        orders: ordersDataSource, selectedStatus: statusTypes.first));
    bool response = await ordersRemote.deleteOrder(orderId: event.id);
    if (response) {
      orders.removeWhere((e) => e.id == event.id);
      ordersDataSource.removeWhere((e) => e.id == event.id);
    }
    emit(OrdersLoaded(
        orders: ordersDataSource, selectedStatus: statusTypes.first));
  }
